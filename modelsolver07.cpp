 ...
 * 作用:
 * 1. 夹层型储层 + 非均布位置 + 非等长裂缝 算法核心。
 * 2. 【核心更新】采用 xf 为距离水平井起点的绝对坐标 [0, L]，并动态映射到无因次复平面。
 * 3. 采用高斯-克朗罗德(Boost Gauss-Kronrod)积分精确处理缝内奇异点，提升稳定性。
 * 4. Stehfest 阶数恢复至防震荡稳定的 N=8 阶。
 */
...
double ModelSolver07::safe_bessel_k_scaled(int v, double x) {
    if (x < 1e-15) x = 1e-15;
...
double ModelSolver07::safe_bessel_i_scaled(int v, double x) {
    if (x < 0) x = -x;
...
ModelSolver07::ModelSolver07(ModelType type)
    ...
    m_currentN(0)
{
    // 【强制设定】初始化 Stehfest 阶数为 8 阶
    precomputeStehfestCoeffs(8);
}

ModelSolver07::~ModelSolver07() {}

void ModelSolver07::setHighPrecision(bool high) {
    m_highPrecision = high;
}

QString ModelSolver07::getModelName(ModelType type, bool verbose) {
    int id = (int)type + 1;
    QString baseName;
    QString subType;
...
QVector<double> ModelSolver07::generateLogTimeSteps(int count, double startExp, double endExp) {
    QVector<double> t;
    if (count <= 0) return t;
    t.reserve(count);
...
ModelCurveData ModelSolver07::calculateTheoreticalCurve(const QMap<QString, double>& params, const QVector<double>& providedTime) {
    QVector<double> tPoints = providedTime;
    if (tPoints.isEmpty()) {
        tPoints = generateLogTimeSteps(100, -3.0, 4.0);
    }
    ...
    double td_coeff = 3.6 * kf / (phi * mu * Ct * std::pow(L_half, 2.0));
    ...
    calcParams["L_total"] = L_total;
    calcParams["L_half"] = L_half;
    ...
    QVector<double> finalP(tPoints.size());
    QVector<double> finalDP(tPoints.size());
...
void ModelSolver07::calculatePDandDeriv(const QVector<double>& tD, const QMap<QString, double>& params,
...
double ModelSolver07::flaplace_composite(double z, const QMap<QString, double>& p) {
    ...
    for (int i = 0; i < n_fracs; ++i) {
        ...
        // 2. 无因次缝位置计算：【核心更新】输入为距起点的绝对距离 [0, L_total]
        ...
        if (p.contains(key_xf_0)) {
            ...
            // 如果没输入，默认生成均匀分布：距离起点 5% ~ 95%
            xf_val = 0.05 * L_total + 0.9 * L_total * (double)i / std::max(1, n_fracs - 1);
        }

        // 【核心映射】：将 [0, L_total] 映射为无因次域 [-1, 1]
    ...
    for (int i = 0; i < n_fracs; ++i) {
        for (int j = 0; j < n_fracs; ++j) {
            ...
            if (i == j) {
                ...
                // Boost 库提供的高效高斯-克朗罗德数值积分，替换了旧版几十行易警告的纯手写循环
