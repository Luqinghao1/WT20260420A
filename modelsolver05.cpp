/*
 * 文件名: ModelSolver05.cpp
 * 文件作用与功能描述:
 * 1. 压裂水平井页岩型复合模型 Group 5 (Model 145-180) 核心计算实现。
 * 2. 基于 ModelSolver02 (等长裂缝) 改造为非等长裂缝版本。
 ...
 * 7. 【2026-03 降噪优化】接入 Boost Gauss-Kronrod 工业级积分器彻底抹平积分噪声，并将 Stehfest 阶数降为 8 阶。
 * 8. 与 ModelSolver02 的关键差异:
 *    - PWD_composite 接收 QVector<double> LfD 向量而非标量
 *    - 各裂缝积分区间独立: [-LfD[j], +LfD[j]]
 *    - 自影响等效观测点: 0.732 * LfD[i] (逐缝独立)
 *    - 归一化因子: 2*LfD[j] (逐列独立)
 *    - 使用 k2 计算 M12: M12 = kf / k2
 *    - 裂缝半长从 Lf_1 ~ Lf_nf 独立读取
 *    - 裂缝位置从 xf_1 ~ xf_nf 读取或默认 linspace(-0.9, 0.9, nf)
 */
...
double ModelSolver05::safe_bessel_k(int v, double x) {
    if (x < 1e-15) x = 1e-15;
    try { return boost::math::cyl_bessel_k(v, x); } catch (...) { return 0.0; }
}

// 功能：计算带有指数缩放的 K_v 函数 (K_v(x) * e^x)，避免大自变量时的数值下溢
double ModelSolver05::safe_bessel_k_scaled(int v, double x) {
    if (x < 1e-15) x = 1e-15;
...
double ModelSolver05::safe_bessel_i_scaled(int v, double x) {
    if (x < 0) x = -x;
...
ModelSolver05::ModelSolver05(ModelType type)
    : m_type(type), m_highPrecision(true), m_currentN(0) {
    precomputeStehfestCoeffs(8); // 预计算默认 8 阶的 Stehfest 反演系数，压制高阶震荡
}

// 析构函数
ModelSolver05::~ModelSolver05() {}

// 功能：设置求解器是否开启高精度模式
void ModelSolver05::setHighPrecision(bool high) { m_highPrecision = high; }

// 功能：获取当前求解器对应的模型中文描述名称
QString ModelSolver05::getModelName(ModelType type, bool verbose)
{
    int id = (int)type + 1;
    QString baseName;
    QString subType;
...
QVector<double> ModelSolver05::generateLogTimeSteps(int count, double startExp, double endExp) {
    QVector<double> t;
    if (count <= 0) return t;
    t.reserve(count);
...
ModelCurveData ModelSolver05::calculateTheoreticalCurve(const QMap<QString, double>& params, const QVector<double>& providedTime)
{
    ...
    auto func = std::bind(&ModelSolver05::flaplace_composite, this, std::placeholders::_1, std::placeholders::_2);
...
void ModelSolver05::calculatePDandDeriv(const QVector<double>& tD, const QMap<QString, double>& params,
...
double ModelSolver05::calc_fs_dual(double u, double omega, double lambda) {
...
double ModelSolver05::calc_fs_shale(double u, double omega, double lambda) {
...
// 功能：Laplace拉氏域综合耦合中心 (非等长裂缝版)
double ModelSolver05::flaplace_composite(double z, const QMap<QString, double>& p) {
...
// 功能：通过半解析边界元计算 DFN (离散裂缝网络) 的基础无因次压力 (非等长裂缝版)
double ModelSolver05::PWD_composite(double z, double fs1, double fs2, double M12, const QVector<double>& LfD, double rmD, double reD,
    ...
    for (int i = 0; i < n_fracs; ++i) {
        for (int j = 0; j < n_fracs; ++j) {
            ...
            if (i == j) {
                // 自影响项：引入无限导流裂缝等效观测点 (0.732 * LfD[i])，逐缝独立
                ...
                // 积分区间: [-LfD[i], x_obs] 和 [x_obs, LfD[i]]，逐缝独立
...
double ModelSolver05::gauss15(std::function<double(double)> f, double a, double b) {
...
double ModelSolver05::adaptiveGauss(std::function<double(double)> f, double a, double b, double eps, int depth, int maxDepth) {
    double c = (a + b) / 2.0;
...
void ModelSolver05::precomputeStehfestCoeffs(int N) {
...
// 功能：拉取特定阶数对应的预存反演权重
double ModelSolver05::getStehfestCoeff(int i, int N) {
...
// 功能：阶乘数学原语
double ModelSolver05::factorial(int n) {
    if(n <= 1) return 1.0;
    double r = 1.0;
