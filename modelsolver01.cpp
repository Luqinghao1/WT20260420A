 ...
 * 作用与功能:
 * 1. 夹层型储层 + 等长裂缝 算法核心。
 * 2. 【核心更新】淘汰了 M12 输入，改为读取 k2 并计算 M12，与 04-09 架构完全统一。
 * 3. 彻底清除了废弃的 gauss15 和 adaptiveGauss 函数，只保留 Boost 库积分。
                                        ...
                                        QVector<double>& outPD, QVector<double>& outDeriv)
{
    ...
    auto calculateSinglePoint = [&](int k) {
        ...
        if (gamaD > 1e-9) {
            ...
            else pd_real = -1.0 / gamaD * std::log(arg_min) - 1.0 / (gamaD * arg_min) * (arg - arg_min);
        }
