 ...
 * 作用:
 * 1. 页岩型储层 + 非均布位置 + 非等长裂缝 算法核心。
 * 2. 【核心更新】采用 xf 为距离水平井起点的绝对坐标 [0, L]，动态映射到无因次域。
 * 3. 严格规范化代码缩进，解除单行多指令导致的编译器警告。
 */
...
void ModelSolver08::setHighPrecision(bool high) {
    m_highPrecision = high;
}

QString ModelSolver08::getModelName(ModelType type, bool verbose) {
    int id = (int)type + 1;
    QString baseName = QString("页岩型储层试井解释模型%1").arg(id);
    QString subType;

    if (id <= 12) subType = "页岩型+页岩型";
    else if (id <= 24) subType = "页岩型+均质";
    else subType = "页岩型+夹层型";
    ...
    auto calculateSinglePoint = [&](int k) {
        ...
        if (gamaD > 1e-9) {
            ...
            if (arg >= arg_min) {
                ...
                pd_real = -1.0 / gamaD * std::log(arg_min) - 1.0 / (gamaD * arg_min) * (arg - arg_min);
            }
        }
...
double ModelSolver08::PWD_composite(double z, double fs1, double fs2, double M12, const QVector<double>& LfD, double rmD, double reD, int n_fracs, const QVector<double>& xwD, ModelType type) {
    int id = (int)type + 1;
    int groupIdx = (id - 1) % 12;
